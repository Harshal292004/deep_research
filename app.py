...
@st.cache_resource
def get_api_client():
    return ResearchAPIClient(base_url=st.secrets.get("BACKEND_URL"))


api_client = get_api_client()

# Initialize session state
# session_id has nothing to do with persistence its just for grouping the current chates under same id
...
def validate_required_keys():
    ...
    missing_keys = [
        ...
        if not st.session_state.api_keys.get(key, "").strip()
    ]
    return len(missing_keys) == 0, missing_keys


def render_api_configuration():
    """Render API key configuration inputs"""
    ...
    with st.expander("API Keys", expanded=True):
        ...
        st.session_state.api_keys["LANGFUSE_HOST"] = st.text_input(
            "Langfuse Host",
            value=st.session_state.api_keys.get(
                "LANGFUSE_HOST", "https://cloud.langfuse.com"
            ),
...
@st.dialog("⚙️ API Configuration")
def show_api_config_dialog():
    """Dialog wrapper for configuration"""
    st.caption("Manage the credentials used by the research pipeline.")
        ...
        st.session_state.chat_history.append(
            {
                ...
                "title": (
                    st.session_state.current_chat_messages[0]["content"][:50] + "..."
...
def delete_chat(chat_id: str):
    ...
    # user deletes the currrent chat he/she is viewing
...
async def run_research_pipeline(query: str):
    ...
    try:
        ...
        result = await api_client.start_research(
            ...
            langfuse_config=(
                {
                    "public_key": st.session_state.api_keys.get(
                        "LANGFUSE_PUBLIC_KEY", ""
                    ),
                    "secret_key": st.session_state.api_keys.get(
                        "LANGFUSE_SECRET_KEY", ""
                    ),
                    "host": st.session_state.api_keys.get(
                        "LANGFUSE_HOST", "https://cloud.langfuse.com"
                    ),
                }
                if st.session_state.api_keys.get("LANGFUSE_PUBLIC_KEY")
                else None
            ),
...
if st.button("⚙️ Configure API Keys", type="secondary"):
    show_api_config_dialog()

# Validate keys to control chat input availability
keys_valid, missing_keys = validate_required_keys()

status_container = st.container()
with status_container:
    if keys_valid:
        st.success("API keys ready. You're good to start researching!")
    else:
        st.warning(f"Missing required keys: {', '.join(missing_keys)}")
...
if keys_valid:
    ...
    st.info(
        "⚠️ Please configure all required API keys using the button above to start researching."
    )
    user_query = None
...
if (
    ...
    and st.session_state.current_chat_messages
    and len(st.session_state.current_chat_messages) >= 2
    and st.session_state.current_chat_messages[-1]["role"] == "assistant"
    and "Processing your research query"
    in st.session_state.current_chat_messages[-1]["content"]
):
    ...
    with st.spinner("🔍 Researching... This may take several minutes."):
        try:
            ...
            if (
                st.session_state.current_chat_messages
                and "Processing your research query"
                in st.session_state.current_chat_messages[-1].get("content", "")
            ):

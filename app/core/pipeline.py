...
class ResearchPipeline:
    ...
    async def _execute_pipeline(self, task_id: str):
        ...
        try:
            ...
            section_graph, research_graph, writer_graph = self.graph_factory.create_graphs(api_keys=task["api_keys"],)
            ...
            if task.get("langfuse_config"):
                import os
                os.environ['LANGFUSE_SECRET_KEY']= task["langfuse_config"].get("secret_key")
                os.environ['LANGFUSE_PUBLIC_KEY']= task["langfuse_config"].get("public_key")
                os.environ['LANGFUSE_BASE_URL']= task["langfuse_config"].get("host", "https://cloud.langfuse.com")
            ...
            from src.models.state import ReportState, ResearchState

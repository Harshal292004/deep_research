...
class ResearchPipeline:
    ...
    async def _execute_pipeline(self, task_id: str):
        ...
        try:
            ...
            section_graph, research_graph, writer_graph = (
                self.graph_factory.create_graphs(
            ...
            if task.get("langfuse_config"):
                ...
                os.environ["LANGFUSE_SECRET_KEY"] = task["langfuse_config"].get(
                    "secret_key"
                )
                os.environ["LANGFUSE_PUBLIC_KEY"] = task["langfuse_config"].get(
                    "public_key"
                )
                os.environ["LANGFUSE_BASE_URL"] = task["langfuse_config"].get(
            ...
            async for state in research_graph.astream(
                {
                    ...
                    "sections": (
                        report_state.sections.sections if report_state.sections else []
                    ),
                },
                stream_mode=["values"],
                config=config_dict,
